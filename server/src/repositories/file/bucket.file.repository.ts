...
export class BucketFileRepository extends StoreBackedRepository<IBucket> implements IBucketRepository {
  ...
  async findByPriority(minPriority: number, maxPriority: number): Promise<IBucket[]> {
    ...
    return allBuckets.filter(
      (bucket) => bucket.priority >= minPriority && bucket.priority <= maxPriority
    );
  }
  ...
  async findUnderfundedBuckets(): Promise<IBucket[]> {
    ...
    return allBuckets.filter((bucket) => {
      ...
      const totalDays = Math.ceil(
        (bucket.targetDate.getTime() - bucket.createdAt.getTime()) / (1000 * 60 * 60 * 24)
      );
      const daysPassed = Math.ceil(
        (today.getTime() - bucket.createdAt.getTime()) / (1000 * 60 * 60 * 24)
      );
  ...
  async findByTargetDateRange(startDate: Date, endDate: Date): Promise<IBucket[]> {
    ...
    return allBuckets.filter(
      (bucket) => !!bucket.targetDate && bucket.targetDate >= startDate && bucket.targetDate <= endDate
    );
  }

...
export interface ICashFlowRepository extends IRepository<ICashFlow> {
  ...
  getCashFlowByCategory(userId: string, startDate: Date, endDate: Date): Promise<{ category: TransactionCategory; total: number }[]>;
  getCashFlowByBucket(userId: string, startDate: Date, endDate: Date): Promise<{ bucketId: string; total: number }[]>;

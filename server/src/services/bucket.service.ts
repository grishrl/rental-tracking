import { IBucket, Bucket, BucketType, BucketStatus } from '../models/bucket.model';
...
export class BucketService {
  ...
  async createBucket(bucketData: Omit<IBucket, 'id' | 'createdAt' | 'updatedAt' | 'progress'>): Promise<IBucket> {
  ...
  async updateBucket(id: string, updates: Partial<Omit<IBucket, 'id' | 'createdAt' | 'updatedAt'>>): Promise<IBucket | null> {
    ...
    if (updates.targetAmount || updates.currentAmount !== undefined) {
      const bucket = new Bucket(existingBucket);
      if (updates.targetAmount) bucket.updateTarget(updates.targetAmount, updates.targetDate);
  ...
  async getBucketById(id: string): Promise<IBucket | null> {
  ...
  async getBucketsByType(type: BucketType): Promise<IBucket[]> {
    return this.bucketRepository.findByType(type);
  }

  async getBucketsByStatus(status: BucketStatus): Promise<IBucket[]> {
    return this.bucketRepository.findByStatus(status);
  }

  async addFundsToBucket(bucketId: string, amount: number, transactionDescription?: string): Promise<IBucket> {
    if (amount <= 0) {
    ...
    const bucketInstance = new Bucket(bucket);
  ...
  async removeFundsFromBucket(bucketId: string, amount: number, transactionDescription?: string): Promise<IBucket> {
    if (amount <= 0) {
      throw new Error('Amount must be positive');
    }
  ...
  async transferFunds(fromBucketId: string, toBucketId: string, amount: number): Promise<{ fromBucket: IBucket; toBucket: IBucket }> {
  ...
  async getBucketSummary(userId: string): Promise<BucketSummary> {
    const buckets = await this.bucketRepository.findByOwner(userId);
  ...
  async getAllocationSuggestions(userId: string, availableAmount: number): Promise<AllocationSuggestion[]> {
    const buckets = await this.bucketRepository.findByOwner(userId);
    const activeBuckets = buckets.filter(b => b.status === 'active');
  ...
  private validateBucketData(data: Omit<IBucket, 'id' | 'createdAt' | 'updatedAt' | 'progress'>): void {

...
exports.RenterFileRepository = void 0;
const store_backed_repository_1 = require("../store-backed.repository");
class RenterFileRepository extends store_backed_repository_1.StoreBackedRepository {
    constructor(store) {
        super(store);
    }
    ...
    async findRentersRequiringFollowUp() {
        const allRenters = await this.findAll();
        return allRenters.filter((renter) => Array.isArray(renter.communicationHistory) &&
    ...
    async searchRenters(query) {
        ...
        if (query.email) {
            ...
            results = results.filter((renter) => renter.email.toLowerCase().includes(emailLower));
        }
        if (query.phoneNumber) {
            ...
            results = results.filter((renter) => {
                ...
                const cleanAltPhone = renter.alternatePhone ? renter.alternatePhone.replace(/\D/g, '') : '';
...
exports.RenterFileRepository = RenterFileRepository;
//# sourceMappingURL=renter.file.repository.js.map